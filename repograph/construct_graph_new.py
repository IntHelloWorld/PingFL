...
from interfaces.d4j import TestFailure, parse_coverage
...
class CodeGraph:
    ...
    def parse_file(self, fname: str, rel_fname: str, lang: str, is_test: bool) -> List[Tag]:
        ...
        for node, tag_type in captures:
            tag = self.create_tag(node, tag_type, fname, rel_fname, code, is_test)
            if tag:
                tags.append(tag)
                
        return tags

    def create_tag(self, node, tag_type: str, fname: str, rel_fname: str, code: str, is_test: bool) -> Optional[Tag]:
        ...
        for test_case in test_class.test_cases:
            ...
            for node in tqdm(test_graph.nodes, desc="Spliting graph"):
                ...
                if node.category == 'field':
                    continue
                flag = False
                if node.rel_fname == 'com/google/javascript/jscomp/PrepareAst.java':
                    if node.category == 'function':
                        if node.name == 'visit':
                            print(f"PrepareAst.java {node.name}#{node.line[0]}-{node.line[1]}")
