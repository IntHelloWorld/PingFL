...
from src.repograph.construct_graph_new import Tag
...
class RepoSearcher:
    ...
    def get_covered_methods_of_class(self, class_name):
        ...
        # fuzzy match
            ...
            if not possible_classes:
                return f"No covered methods in the class {class_name} were found. It was not covered by the failing tests. Please try something else."
            result = {}
    ...
    def get_method_code(self, class_name, method_name):
        ...
        # fuzzy match
            ...
            possible_classes = [c for c in self.method_dict if class_name in c]
            if not possible_classes:
                return f"No covered methods in the class {class_name} were found. It cannot be the buggy method. Please try something else."
            for c in possible_classes:
                for m in self.method_dict[c]:
                    if m.name == method_name:
                        methods.append(m)
        if not methods:
            return f"No covered methods with the name {method_name} were found. It cannot be the buggy method. Please try something else."
