...
class Memory:
    ...
    def serialize(self) -> str:
        ...
        for message in self.messages:
            if isinstance(message, ChatCompletionMessage):
                ...
                tokens = sum(
                    [
                        self.get_tool_call_tokens(toolcall)
                        for toolcall in message.tool_calls
                    ]
                )
    ...
    def get_tool_call_tokens(
        self, tool_call: ChatCompletionMessageToolCall
    ) -> int:
        return len(self.encoding.encode(tool_call.function.name)) + len(
            self.encoding.encode(tool_call.function.arguments)
        )
