...
def get_relative_distance(
    ...
    """
    graph_files = [
        (bug_info.bug_path / t / "repograph.pkl") for t in test_names
    ]
    all_distances = []
    for graph_file in graph_files:
                ...
                for m in should_find_methods:
                    if m.class_name == method_node.outer_class:
                        if m.name == method_node.name:
                            if (
                                m.loc[0][0] <= method_node.line[0]
                                and m.loc[1][0] >= method_node.line[1]
                            ):
        ...
        ), f"some buggy methods not found in {graph_file}"
        assert (
            predict_node is not None
        ), f"predict method not found in {graph_file}"
...
def get_distance(bug_info: BugInfo, test_failure_obj: TestFailure):
    ...
    for ranked_method in ranked_methods:
        ...
        assert (
            method_id in method_to_test
        ), f"{method_id} not in verify results, please check"
        test_names = method_to_test[method_id]

        distances = get_relative_distance(
            test_names, modified_methods, method_id
        )
        if distances:
...
def print_result(bug_names, config_file):
    ...
    for proj in output:
        for i in [1, 3, 5]:
            output[proj][f"RD@{i}"] = sum(output[proj][f"RD@{i}"]) / len(
                output[proj][f"RD@{i}"]
            )
