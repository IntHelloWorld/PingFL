...
{edit_suggestion}Start verifying the suspicious method by calling the `edit_and_run` function.
"""

EDIT_SUGGESTION = """The part of the method that needs to be checked:
```
{explaination}
```\n\n"""
...
You are a software debugging assistant. Given the hypotheses about potential bugs in a method, your task is to verify that if this method is the best repair location.

Special Instructions:
1. Fully consider the data flow and calling relationship between methods, the provided method is verified as buggy only if it is the best place to fix the bug
2. Focus on concrete evidence rather than speculation
"""
...
Start verifying the suspicious method based on the above information"
"""


class MethodVerifyResult(BaseModel):
    category: str = Field(
        title="the category of the method",
        description="A string indicating the category of the method. 'BUGGY' if the method is the best repair location, 'NOT_BUGGY' if the method is not the best repair location, 'UNCERTAIN' if you cannot make a judgment based on the current information."
    )
    explanation: str = Field(
        title="the explanation of the method",
        description="If the method is 'BUGGY', provide a detailed explanation of the bug. If the method is 'NOT_BUGGY', explain the reason and provide suggestiones for the subsequent debugging process. If the method is 'UNCERTAIN', explain which part of this method needs further inspection."
    )