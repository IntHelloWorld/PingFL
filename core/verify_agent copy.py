...
from interfaces.d4j import check_out_playground, run_single_test_playground
from repograph.graph_searcher import Tag
...
class VerifyAgent:
    ...
    def create_verifier_memory(
        ...
        verifier_memory.add_message({
            "role": "system",
            "content": VERIFY_AGENT_VERIFIER_USER_PROMPT.format(**input_dict)
        })
    ...
    def create_editor_memory(self, process: ProcessState) -> Memory:
        ...
        if len(process.memories) == 0:
            ...
            explaination = process.memories[-1].get_messages()[-1].parsed["explanation"]
        ...
        editor_memory.add_message({
            "role": "system",
            "content": VERIFY_AGENT_EDITOR_USER_PROMPT.format(**input_dict)
        })
    ...
    def run_process(
        ...
        while process.edit_count < self.max_edit_times:
            ...
            new_method_code, edit_result = edit_and_run(self.bug_info, process, edit_commands)
            ...
            process.memories.append(
                self.create_verifier_memory(process, new_method_code, edit_result)
            )
    ...
    def parse_results(self, verify_inputs: List[VerifyInput]) -> List[VerifyResult]:
...
def edit_and_run(bug_info: BugInfo, process: ProcessState, edit_commands: List[str]) -> Tuple[str, str]:
    ...
    new_method_code = "\n".join(new_content.splitlines()[method.line[0]-2:method.line[1]+extra_lines])
    ...
    # run the test
    run_single_test_playground(bug_info, playgroud_dir, process.verify_input.test_name)
